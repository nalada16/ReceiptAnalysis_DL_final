...
→ user 0 本月**大幅多花在正餐主食**（+1162）：均價從 115 跳到 256（+123%）。拆開來看，
平常吃的是 99 元的嫩切雞肉堡，但這月**多了一筆 1199 元的韓式烤肉 2 人套餐**（聚餐性質），
單筆高價把整個類型均價拉高——屬於「**一次性高價聚餐**」而非系統性漲價。
...
| **④ 「變貴」可能來自一次性高價聚餐** | user 0 正餐主食均價 115→256：是多買了一筆 1199 元韓式烤肉套餐（聚餐），非單品系統性漲價 → 提醒解讀時要分辨「真漲價」vs「偶發高價」 |
...
重點摘錄（`insight_user0_upgrade.png`）：
- **user 0 的便宜類別長期走低**：超商零食點心 **−46%**、優酪乳乳製品 **−40%**（穩定下降）。
- **user 2 瓶裝茶飲 +144%**（36→88 元）：長期從超商普通飲料換成較貴品牌的「品質升級」型通膨。

**完整版**：每位 user「所有通過門檻超類型」的單價成長歷史（>7 個自動分兩張子圖）：
`outputs/history_user{0,1,2}.png`。以 user 1 為例（14 個超類型）：

![user1 完整單價歷史](outputs/history_user1.png)
...
- **「變貴」需分辨真漲價 vs 偶發高價**：超類型均價含「換買更貴品項/一次性高價聚餐」，
  解讀漲幅時應回查成員品項（如 user 0 正餐的 +123% 來自單筆 1199 元烤肉套餐）。
